...
import { formatShortDate, formatFileSize, cn } from '@/lib/utils';
...
export default function DocumentsPage() {
  ...
  const handleUpload = () => {
    ...
    // Pass the file to addDocument for Supabase Storage upload
    addDocument(
      {
  ...
  return (
    <div className="space-y-6">
                  ...
                  <tr key={doc.id} className="border-b border-[#F3F4F6] hover:bg-[#F9FAFB]">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="sm" onClick={() => handlePreview(doc)}>Preview</Button>
      ...
      {/* Preview modal */}
      <Modal isOpen={!!previewDoc} onClose={() => { setPreviewDoc(null); setPreviewUrl(null); }} title={previewDoc?.name ?? 'Preview'} maxWidth="max-w-2xl">
        {previewDoc && (
          <div className="space-y-4">
            ...
            {previewUrl && previewDoc.fileType.startsWith('image/') ? (
              <img src={previewUrl} alt={previewDoc.name} className="max-w-full rounded-lg border border-[#E5E7EB]" />
            ) : previewUrl && previewDoc.fileType === 'application/pdf' ? (
              <iframe src={previewUrl} className="w-full h-96 rounded-lg border border-[#E5E7EB]" title={previewDoc.name} />
            ) : (
